...
*              from the bitstream.
*
*              XXX This is not not yet fully implemented and tested, this is
*              XXX left as an excercise to the reader.
*
******************************************************************************/
...
MPG_Huffman_Decode (UINT32 table_num, INT32 *x, INT32 *y, INT32 *v, INT32 *w)
{
  ...
  /* Start reading the Huffman code word,  */
  do {

    bits = MPG_Get_Main_Bits (size);

    point += bits;

    /* Set size which is either the code word size or the number of bits to
     * read for the next iteration */
    size = htptr[point] & 0x6000;
        
    /* Check for hit/miss */
    if (htptr[point] & 0x8000) { /* Hit */

      /* Hit, see if we have to back up */
      if (size < 4) {
        /* Back up bitstream 4-size bits */
        currpos = MPG_Get_Main_Pos ();
        MPG_Set_Main_Pos (currpos - (4 - size));
      }
      ...
      *y = htptr[point] & 0xf;
      ...
      /* Set point to start of next table */
      point += htptr[point] & 0xfff;
    }
  ...
  /* Check for error. */
  if (error) {
    ERR ("Illegal Huff code in data. point = %d. tab = %d.",
	 point, table_num);
    *x = *y = 0;
  }
