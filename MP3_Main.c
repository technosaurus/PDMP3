...
* Description: This file contains the top-level functions for reading MPEG1
...
#ifdef OUTPUT_DBG
    dmp_si (&g_frame_header, &g_side_info);
#endif /* OUTPUT_DBG */
...
MPG_Read_Audio_L3 (void)
{
  ...
  if (MPG_Get_Filepos () == C_MPG_EOF) return (ERROR);
...
MPG_Get_Main_Data (uint32_t main_data_size, uint32_t main_data_begin)
{
  ...
  if (main_data_begin > g_main_data_top) {
     ...
     */
    (void) MPG_Get_Bytes (main_data_size, &(g_main_data_vec[g_main_data_top]));
  ...
  for (i = 0; i < main_data_begin; i++) {
    g_main_data_vec[i] = 
      g_main_data_vec[g_main_data_top - main_data_begin + i];
  }

  start_pos = MPG_Get_Filepos ();
  
  /* Read the main_data from file */
  (void) MPG_Get_Bytes (main_data_size, &(g_main_data_vec[main_data_begin]));
